...
def main() -> None:
    ...
    epoch = (
        metrics.get("epoch")
        or metrics.get("Epoch")
        or list(range(1, len(next(iter(metrics.values()))) + 1))
    )
    ...
    # Relative error
    train_rel = (
        metrics.get("avg_train_relative_error")
        or metrics.get("train_relative_error")
        or []
    )
    val_rel = (
        metrics.get("avg_val_relative_error") or metrics.get("val_relative_error") or []
    )
